...
// AJAX requests
function OnGameChange(title="Breath of the wild", async=true) {
    ToggleLoading(false);
...
function ChangeVideo(req) {
    document.getElementById("youtube-video").src = "https://www.youtube.com/embed/"+req.items[0].id.videoId;
}
...
function SetPlatforms(plats) {
    ...
    for(var i=0; i<plats.length; i++) {
        if(plats[i]=="Xbox 360" || plats[i]=="Xbox One")
            ...
            $("#pc").show();
        else if(plats[i]=="Nintendo Switch")
            $("#switch").show();
