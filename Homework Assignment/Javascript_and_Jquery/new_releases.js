...
$(document).ready(function() {
        GenericPost("api.php", genHead(), SetNewRel);
});

function SetNewRel(data) {
    games = data["data"];
    setGame();
}

function setGame() {
    $("#gametitle").html(games[focus]["title"]);
    $("#hovtitle").html(games[focus]["title"]);
    $("#score").html(games[focus]["metacritic"]);
    $("#Desc").html(games[focus]["description"]);
    $("#weblink").attr("href",games[focus]["site"]);
    $("#Genres").html(games[focus]["genres"]);
    setPegi(games[focus]["ESRB_rating"])
    setPlatforms(games[focus]["platforms"]);
    setArt(games[focus]["artwork"]);
}
...
function setPegi(rating) {
    ...
    else if(rating=="everyone-10-plus")
...
function setPlatforms(data) {
    ...
    for(var i=0; i<data.length-1; i++) {
        //console.log(data[i]);
        if(data[i]=="PC" || data[i]=="Linux" || data[i]=="macOS" || "PC (Windows)") {
            $(".fa-desktop").show();
            pc=true;
            setSystReqs(data[i]);
        }
        if(data[i]=="Xbox" || data[i]=="Xbox Series S/X" || data[i]=="Xbox One")
            $(".fa-xbox").show();
        if(data[i]=="PlayStation 4" || data[i]=="PlayStation 5" || data[i]=="Playstation")
            $(".fa-playstation").show();
        if(data[i]=="Nintendo Switch" || data[i]=="Nintendo Wii")
            $(".iconify").show();
    }
    if(!pc)
        setSystReqs(null);
